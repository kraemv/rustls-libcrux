    ...
    T: SigningKey + Clone + Debug + 'static,
{
    ...
    ) -> Result<Arc<dyn rustls::sign::SigningKey>, pkcs8::Error> {
    ...
    fn load_private_key(
        ...
        let key = match key_der {
            PrivateKeyDer::Pkcs8(der) => {
                type PkInfoType<'a> =
                    PrivateKeyInfo<Any, OctetString, BitString, SetOfRef<'a, Attribute>>;
                ...
                match algo_oid_arcs.as_slice() {
                    ...
                    [1, 2, 840, 10045, 2, 1] => {
                        let parameter_oid: ObjectIdentifier = private_key_info
                            ...
                            .to_ref()
                            .try_into()
                            .map_err(|_| to_rustls_error(pkcs8::Error::KeyMalformed))?;
